...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		querytree_list = pg_analyze_and_rewrite(parsetree, query_string,
												NULL, 0);
		//add by xuliang 170703
		if(nodeTag(parsetree) == T_DeleteStmt|| nodeTag(parsetree) == T_UpdateStmt)
     	{
	 	  return;
	    }
						  ...
						  const char **dbname)
{
    ...
    SetConfigOption("search_path", "pg_catalog", ctx, gucsource);
    SetConfigOption("exit_on_error", "true", ctx, gucsource);
