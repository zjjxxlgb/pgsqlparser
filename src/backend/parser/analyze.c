...
transformStmt(ParseState *pstate, Node *parseTree)
{
	...
	switch (nodeTag(parseTree))
	{
		...
		case T_DeleteStmt:
		     //modify by xuliang begin 20170703
	        ereport(LOG, (errmsg("dbtablename %s", ((DeleteStmt *) parseTree)->relation->relname)));
		...
		case T_UpdateStmt:
			ereport(LOG, (errmsg("dbtablename %s", ((UpdateStmt *) parseTree)->relation->relname)));
			result = makeNode(Query);
			result->commandType = CMD_UTILITY;
			result->utilityStmt = (Node *) parseTree;
